...
## Что проверить на приставке (ничего из этого эмулятор не докажет)

PPSSPP прощает то, за что железо наказывает. Список составлен по местам, где код
опирается на поведение железа, а проверен только в эмуляторе.

1. **Кэш данных.** Вершины мешей пишет CPU, читает GE. Признак беды: мусорные
   треугольники или «взрыв» геометрии на первом кадре острова. Место: `plat_gpu_writeback`
   в `core/mesh.c` (на PSP — `sceKernelDcacheWritebackRange`).
2. **Звук.** Колбэк `pspaudiolib` должен укладываться в буфер: 1024 кадра при 44,1 кГц —
   это 23 мс. Признак беды: треск, щелчки, залипший тон. Проверять при открытии большого
   глаза (арпеджио плюс эмбиент плюс шаги). Оценка расхода CPU в `core/audio.c` —
   расчётная, 2,5–5,5 % на 222 МГц.
3. **Частота кадров.** Бюджет: 8000 треугольников и 100 draw-call. Реальные кадры —
   2700–4200 треугольников, 20–30 вызовов. Оверлей по SELECT показывает fps, мкс CPU
   и GPU, кучу. Если ниже 55 fps где-то в Саду Зеркал — виноват проход спрайтов (луч),
   уменьшать `BEAM_SPRITES_MAX`.
4. **Память.** `PSP_HEAP_SIZE_KB(16384)`. На PSP-1000 (32 МБ) запас проверять по оверлею:
   куча после загрузки острова должна оставаться выше 4 МБ.
5. **Сохранение.** `save.bin` пишется рядом с EBOOT через временный файл и `sceIoRename`.
   Проверить: открыть глаз, выключить приставку кнопкой, включить — прогресс на месте.
6. **Кнопка Home и спящий режим.** `sys_psp.c` ставит коллбэки выхода; suspend/resume
   с `scePowerRegisterCallback` не реализован — после сна звук может не ожить.
7. **Ввод.** Аналоговый стик на разных ревизиях имеет разный мёртвый ход; в игре порог
   задан в `core/player.c`. Если Око «плывёт» само — поднять порог.
