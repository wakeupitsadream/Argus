...
## Решения и допущения вехи 6–7: свет, небо, сегменты (2026-09-22)

1. **Цветной свет вместо серого ambient.** Было: `материал × (ambient + N·L)` плюс подмешивание
   тона тени. Любая палитра сходилась к серо-сиреневому, а грани, отвёрнутые от солнца,
   проваливались в чёрный. Стало: два источника из палитры региона — `sun` (тёплый прямой) и
   `sky` (холодная заливка), материал умножается на цвет света (`core/mesh.c`). Слоты палитры
   переопределены как альбедо и потому заметно светлее прежних. Формат APAL поднят до версии 2.
2. **Мягкий спад за терминатором** (`LIGHT_WRAP = 0,22`): честный ламберт на кубической
   геометрии даёт восемь ориентаций, из которых половина уходит в ноль. Заворот света
   оставляет отвёрнутой грани десятую долю прямого — боковины перестают быть дырами.
3. **Дальний план — 2D, а не геометрия.** Силуэты чужих островов рисуются после неба
   плоскими треугольниками с параллаксом от поворота камеры. Настоящая геометрия в тумане
   стоила бы треугольников и не дала бы ничего: на таком удалении остров — пятно.
   Цвет считается относительно неба, на котором остров висит (воздушная перспектива),
   а не подмешиванием цвета зенита: на тёмном небе это давало чёрные наклейки.
4. **Облачное море** заполняет нижнюю половину кадра там, где раньше был пустой градиент.
   Оно же прячет нижний край дальнего плана.
5. **Плиты сегментов.** Поворот сегмента менял только проходимость: камень стоял на месте.
   Теперь клетки сегментов едут отдельными мешами (`<уровень>_seg<номер>_<состояние>.msh`),
   поднятыми или утопленными по текущему состоянию. Точка привязки лежит в заголовке AMSH
   (версия 2) — оттуда бьёт пыль, когда плита встаёт на место.
6. **Спящие стражи занимают клетку.** До этого статуя ходила, но сквозь неё можно было
   пройти, и механика взгляда оставалась украшением. Третий параметр задаёт начальную фазу:
   без неё стражи ходят синхронно и коридор либо всегда открыт, либо всегда закрыт.
   Запереть проход навсегда страж не может: без взгляда он не останавливается никогда.
7. **`tools/reach.py`** обходит остров по правилам `core/walk.c` и входит в `make test`.
   Первое, что он нашёл, — что шаг вверх берётся до 0,55, а не до 0,3, как было записано
   в плане: половина «непроходимых» уступов оказалась проходимой.
8. **Западня шлюза.** Стоять вплотную к плоту законно, пока плот не выше шага. Шлюз
   поднимает его на целую ступень — и проба круга с той стороны упирается в стену при
   любом направлении движения: Око замирало навсегда в метре от берега. Лечится
   `player_unstick` (каждый кадр): если точка стала незаконной, Око сдвигается в центр
   своей клетки, а если занят и он — в центр ближайшей соседней. Это общий случай: так же
   мир меняется под ногами при повороте сегмента.
9. **Подпись острова** переходит на меньший кегль, если не влезает в кадр (`CAPTION_MAX_W`).

9. **Блум — единственный пост-эффект.** Кадровый буфер читается как текстура 512×512,
   уменьшается в буфер 128×64, из него вычитается порог (`GU_REVERSE_SUBTRACT`), два
   прохода пинг-понга размывают крестом, результат ложится аддитивом с билинейным
   растяжением. Порог покомпонентный, поэтому высокий (192): низкий пропускал розовый
   горизонт одним красным каналом. Семь маленьких вызовов, 64 КБ VRAM.

## Решения вехи 7: головоломки и сюжет (2026-09-23)

1. **Плита «только под груз»** (`PLATE.params[0] = 1`): Око её не продавливает. Рецензия
   нашла, что финал Террас открывался шагом на плиту, а блок был декорацией. Правило
   в `plates_update`, тест `test_mech_heavy_plate`.
2. **Порядок панели памяти показывают постаменты** (`trigger_glow` в `core/game.c`):
   пока панель не собрана, её последовательность играет по кругу, и постамент вспыхивает
   на своём такте. Так «память» стала памятью, а не перебором шести порядков.
3. **Состояние мира переживает смену острова.** Рычаги и панели памяти хранятся во
   флагах мира — без этого возвраты запирали игрока (Террасы-1, Библиотека-1).
4. **Сюжет ведут камни и отголоски.** У каждого региона своя часть ответа на вопрос,
   почему Аргус закрыл глаза: зеркала — свет шёл, куда велели; террасы — он считал
   обороты, чтобы не уснуть; вода — мир и без надзора знает дорогу; библиотека — всё
   записано, читать некому. Бонусный остров договаривает прямо, финалы — выбор игрока.
   У отголосков террас и воды свои реплики просьбы, прогресса и благодарности.
...
7. **Блум.** Кадровый буфер читается как текстура в том же дисплей-листе, где в него
   рисовали (`draw_bloom` в `gu_render.c`). В PPSSPP это работает без `sceGuTexSync`;
   на железе GE может отдать кэшированные текселы. Признак — мусор или чёрные полосы
   в свечении. Лечение: `sceGuTexSync()` после `set_target`, либо `sceGuCopyImage`
   кадра в отдельный буфер перед чтением.
8. **Ввод.** Аналоговый стик на разных ревизиях имеет разный мёртвый ход; в игре порог
